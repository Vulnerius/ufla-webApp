...
class Reporting(templateConfig: Element, reportingConfig: Element, private val messages: MutableList<String>) {

    private var sheetPower by Delegates.notNull<Double>()
    private var flawsPerFile by Delegates.notNull<Int>()
    private val config: Element = reportingConfig
    ...
    fun start(name: String) {
        ...
        try {
            ...
            Main.controllerK.reportError(e.message.toString() + messages[15])
        }
    }
            ...
            .forEachIndexed { rIdx, row ->
                row.forEachIndexed innerFor@{ cIdx, element ->
                    when {
                        ...
                        element.cellType.equals(CellType.STRING) && element.stringCellValue.lowercase() == config.getAttribute(
                            "HeaderString"
    ...
    private fun setCellValue(toFill: XSSFSheet, frequency: String, activeReleasePerformance: Double) {
        if (toFill.sheetName.equals(templateC.getAttribute("NameOfFirstSheet"))) throw Exception(messages[18])
